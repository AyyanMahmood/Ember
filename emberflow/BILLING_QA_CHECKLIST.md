...
- **Official policy = 15-day grace (see `SUPPORT_PLAYBOOK.md` → Grace Period).** Important technical nuance, stated honestly: EmberFlow's *entitlement* does not read Polar's grace-period dropdown at all — the code grants Pro for the entire `past_due` window (until Polar revokes after its retry schedule, ~21 days), because `past_due ∈ hasAccessGrantingStatus`. So EmberFlow already honors "at least 15 days" with margin. Polar's dropdown (Immediately / 2 / 7 / 14 / 21 — 15 is not an offered value) only governs Polar's *own* benefit system + the tone of its dunning emails, which EmberFlow doesn't use for entitlement. **Config action for launch:** set the Polar dashboard grace to the offered value that doesn't undercut the 15-day policy — i.e. **21 days** — so Polar's own emails don't tell a customer they're revoked before EmberFlow's 15-day commitment is up. This aligns Polar's knob *to* the 15-day policy; it does not change the policy.
...
- [ ] Confirm the Polar dashboard grace-period setting is 21 days (the offered value that satisfies the 15-day policy — see above), so Polar's own dunning emails don't contradict EmberFlow's 15-day access commitment
