...
- [ ] Confirmed via Polar's own docs: subscriptions lock in the price they were created with — there is no in-place plan swap that preserves entitlement without a new checkout. EmberFlow's UI already reflects this correctly (cancel current plan → resubscribe to the new cadence via Manage billing). **Do not build in-place switching** — this is Polar's real constraint, not a gap in our code.
- [ ] Cancel Monthly (`cancel_at_period_end=true`) → confirm the "Switch billing cadence" card explains this correctly → once the old plan actually ends (or via a fresh Free-state checkout), Upgrade → Yearly succeeds
