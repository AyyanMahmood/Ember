...
2. **Login/Signup rebuilt as a Raycast-style two-column layout, Ember-themed** — user explicitly asked for this after seeing a Raycast signup screenshot, reversing the earlier Batch 3 call to skip it. Left column (`.auth-showcase`, visible only ≥960px, hidden and single-column below that) shows **real product content, not invented marketing copy**: reuses the landing page's `.lp-glimpse` hero card verbatim (float animation switched off here — fine on the marketing page, too "showcase-y" for a calm sign-in screen) plus 3 real features. Feature copy was previously hardcoded inline in `LandingPage.jsx`; extracted to `frontend/src/data/features.js` (same centralization pattern as `currencies.js`/`countries.js`) so both pages share one source of truth instead of drifting. Right column (the form) is unchanged from Batch 3. `ForgotPasswordPage`/`ResetPasswordPage` were **intentionally left alone** — user asked specifically about login/signup. **Not yet confirmed on-device** — ask user to check both the ≥960px two-column view and the <960px single-column fallback.
