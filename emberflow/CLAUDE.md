...
- Note: did **not** build a Raycast-style two-column marketing-panel layout (left illustration/carousel side) — that would mean inventing marketing copy/illustration content that doesn't exist yet, which is out of scope for a QA-mode polish pass. What was reused from the Raycast reference was the *icon-row OAuth pattern* and *ToS disclaimer placement*, which map directly onto the single-column card EmberFlow already has.
