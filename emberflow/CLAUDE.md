...
| Premium experience milestone (2026-07-31): activation, loaders, entrance, badge | Complete, build green + **render-verified in headless Chrome (light + dark)**. `61de978`..`4dc0ad4`. Apple-style **Pro activation** (traced ember ring → welcome), **contextual loader family** (EmberSpinner + RouteProgress top-bar + BrandLoader; no skeletons — standing rule honored), **logo-anchored app entrance**, **Early Supporter badge**, **EmberLogo** (real asset + monogram fallback). EmberSpinner + RouteProgress extracted to Ember UI. **Needs the real logo dropped at `frontend/public/emberflow-logo.svg`.** See "Premium Experience Milestone" section below. |
...
> **⚙️ ACTION NEEDED — drop the real logo:** everything brand-facing renders through `EmberLogo`, which loads `frontend/public/emberflow-logo.svg` and falls back to a built-in ember-flame monogram (`EmberMark`) until that file exists. **Place the real logo at `frontend/public/emberflow-logo.svg`** (SVG preferred; a caller can override the path via `EmberLogo`'s `src` prop) and it appears everywhere automatically — activation, app entrance, sidebar brand, auth loader. Nothing renders broken in the meantime.
...
**Files changed:** new — `ProActivation.jsx`, `AppEntrance.jsx`, `ui/EmberLogo.jsx`, `utils/earlySupporter.js`, `styles/components/brand.css`; modified — `ui/Loading.jsx`, `ui/Badge.jsx`, `AppLayout.jsx`, `SubscriptionsPage.jsx`, `App.jsx`, `AuthCallbackPage.jsx`, `styles/index.css`, `styles/components/{loading,badges,sidebar}.css`.
