...
| Launch Hardening Sprint — Phase 1: Billing Lifecycle Audit (2026-07-30) | Complete — 3 real bugs found and fixed (checkout error sanitization, a refund-policy contradiction, the cancellation-stale-state root cause), refund/failed-payment recommendations produced (not implemented, by design), `BILLING_QA_CHECKLIST.md` created. Still gated on a live Polar sandbox test this environment can't run. See "Launch Hardening Sprint — Phase 1" section below. |
...
**Status (2026-07-30, Launch Hardening Phase 1):** root cause partially confirmed and fixed; one half remains unverifiable without a live test. See the "Launch Hardening Sprint — Phase 1" section below for the full audit. Summary: the DB/RLS/webhook-handler layer was checked in detail and is correct (unique index on `user_id`, correct owner-only RLS, `normalizeSubscription()`'s status derivation matches Polar's real lifecycle verified against Polar's own docs). The confirmed, fixed half: `useSubscription.js` only fetched once on mount, and `manageBilling()` navigates the whole tab to Polar's portal (`window.location.assign`, not a new tab) — returning via the back button is a bfcache restore that replays stale React state instead of remounting. Fixed (`5a12fa3`) by refetching on `pageshow`/`visibilitychange`. The **unverified half**: whether the Polar dashboard's webhook endpoint has every relevant `subscription.*` event actually selected for a portal-initiated cancel — this needs a live sandbox test against Vercel's function logs, which this environment can't run. See `BILLING_QA_CHECKLIST.md` §5.
...
- Downgrade (Yearly→Monthly, Monthly→Free): confirmed via Polar's docs that subscriptions lock in their price at creation — there's an update-subscription API, but it can't change an existing subscription onto different pricing. EmberFlow's existing UI copy ("cancel, then resubscribe to the new cadence") is the practically-correct guidance; **no custom logic needed, Polar's own constraint already matches what the UI says.**
...
3. `5a12fa3` — root cause of "stays Pro after cancelling in the portal," see the Known Deferred Issue section above for the full writeup. `useSubscription.js` now refetches on `pageshow`(bfcache restore)/`visibilitychange`(tab refocus).
...
- Failed-payment UX messaging (above) — not launch-blocking, but noticeable in a real user's account if it happens before it's built.
