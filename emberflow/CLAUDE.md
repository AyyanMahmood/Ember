...
| **White Label Foundation session (2026-08-04)** | Docs/ops-only — no EmberFlow product code touched. Two parallel efforts, both outside this repo except for the repo-hygiene item below: (1) **White Label Edition** (`~/Desktop/White Label EmberFlow/`) — found the buyer-facing docs/license/fingerprinting/component-catalog already substantially built from a prior session; added the missing internal go-to-market roadmap, the `sanitize-source.mjs` and `package-for-delivery.mjs` tooling `stamp-license.mjs` referenced but never had, and the buyer delivery workflow — both new scripts tested end-to-end against a scratch copy. (2) **Ember Holdings company ops** (new `~/Desktop/Ember Holdings/`) — launch marketing checklist + support/bug, release/versioning/changelog, customer communication, and refund/billing-support policy docs, framed as reusable company policy with EmberFlow's own docs as the worked examples. (3) **This repo's documentation cleanup** (item 6 of that session's scope, judged low-risk/git-reversible and directly requested): executed the previously-frozen `REPO_CLEANUP_LIST.md` plan now that launch is stable — 3 empty stub files removed, `AGENTS.md` filled with a `CLAUDE.md` pointer, 9 historical/superseded docs moved to new `docs/archive/` (the original 6 plus `BILLING_QA_CHECKLIST.md`, `BILLING_UX_ARCHITECTURE.md`, and `KNOWN_LIMITATIONS.md`, the last after a genuine merge — it and `KNOWN_ISSUES.md` had drifted apart with real items missing from each, now unified in `KNOWN_ISSUES.md`). New `DOCUMENTATION_MAP.md` is the resulting one-page doc index. Root markdown count: 36 → 25. At the time, next priority was V1.5 Phase 1 — EmberFlow Control Center; **superseded by the V2 planning suite below**, see the RESUME HERE marker at the end of this file. |
| **V2 planning suite (2026-08-06)** | Complete, docs-only, committed (`14250e2`, on branch `v2`). Full pre-V2 audit across frontend/API/database/design, produced as four companion documents: `V2_MASTER_AUDIT.md` (10 Must-Fix, 6 Security, 30 Tech Debt, 5 Perf, 8 UX, 5 Mobile, 10 A11y, 0 open SEO defects, 5 Future Ideas explicitly out of scope — **supersedes `KNOWN_ISSUES.md`** as the canonical issue tracker), `V2_ARCHITECTURE.md` (structural review — folder structure, every major flow, coupling/duplication findings), `V2_DESIGN_GUIDE.md` (design-token/component consistency audit against every stylesheet), and `V2_ROADMAP.md` (the 11-phase execution plan synthesizing all three, with a critical-path and parallelization section). No application code was changed to produce any of these. Also scaffolds `.claude/skills/` (12 topic-stub files for distilled, reusable knowledge — separate from `/references`' raw study material). Separately, a pre-existing typo in `vite.config.js`'s dev-proxy fallback error message (`is'nt` → `isn't`) was fixed, build-verified, and committed on its own (`2d6d278`) — unrelated to the V2 docs. **V2 is now the active initiative on the `v2` branch — see the RESUME HERE marker.** No `V2_ROADMAP.md` phase has been started. |
...
## ⏸️ RESUME HERE — V2: Phase 1, Foundation (Data & Schema Integrity)

**Active initiative: V2**, on branch `v2`. **V1 Launch remains ✔ Production Stable** — nothing about V2 reopens or destabilizes it; V2 is the next major body of work, planned but not yet started. The pre-V2 remaining launch gates are unchanged and are the same ones now tracked as `V2_ROADMAP.md`'s MF-11–15: confirm the Vercel/Polar environment-variable checklist, update the Supabase Reset Password email template, apply migration `012_delete_account.sql` to production if not already done, and run the still-outstanding live Polar sandbox end-to-end test (the standing gate noted throughout this file since the original Paddle → Polar migration).

**Start here:** `V2_ROADMAP.md`. It is the single entry point for resuming V2 — read it first, not this section. It sequences the full 11-phase plan (Foundation → Design System → Billing → Invoices → Payments → Proposals → Dashboard/Clients → Settings → Analytics → Polish), cites the exact finding IDs behind every line item, and lays out the critical path and what can run in parallel. Its three companion documents are the source material and don't need to be re-read to resume: `V2_MASTER_AUDIT.md` (the findings — Must-Fix/Security/Tech-Debt/Perf/UX/Mobile/A11y, supersedes `KNOWN_ISSUES.md`), `V2_ARCHITECTURE.md` (structural review), `V2_DESIGN_GUIDE.md` (design-system consistency audit). All four are complete and committed (`14250e2`); no application code has been changed as a result of them.

**Next priority: `V2_ROADMAP.md` → Phase 1 — Foundation: Data & Schema Integrity.** The only phase with no dependencies; it blocks Phases 4–7 (Billing, Invoices, Payments, Proposals). **Do not begin implementation** until this is deliberately picked up — same STOP-and-confirm rule as everywhere else in this file, and doubly so here since Phase 1 adds constraints to `payments`/`proposal_items`, tables already holding production financial data. In parallel, independent of engineering: the five launch-verification gates (MF-11–MF-15, inside Phase 2) — Vercel/Polar/Supabase config confirmation and the still-outstanding live Polar sandbox end-to-end test — can start any time and have no dependency on Phase 1.

**What remains before Phase 1 can begin:** nothing structurally — the plan is complete and the schema/finding groundwork is documented. What's needed is an explicit go-ahead to start writing the Phase 1 migration (new `CHECK`/`NOT NULL`/index/RLS-consolidation migration per `V2_ROADMAP.md` Phase 1), plus, per that phase's own note, verifying against production data first that no existing `payments` row would violate the new `amount > 0` constraint before the migration is applied.

**Deferred, not abandoned — V1.5 Phase 1: EmberFlow Control Center.** Still a valid, separately-scoped initiative; layered behind V2's foundation phases rather than superseded by them. Pick it up only if explicitly redirected there instead of V2.
