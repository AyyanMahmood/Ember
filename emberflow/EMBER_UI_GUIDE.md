...
| `components/modal-dialog/` | Component | Icon-medallion feature (HeroUI `AlertDialog.Icon`-inspired) extracted 2026-07-30 and applied to EmberFlow's existing shared Modal/ConfirmDialog, retroactively upgrading every confirm dialog in the app. The modal/focus-trap/drawer mechanics predate this pass and weren't re-researched against references — flagged as a follow-up. **2026-07-31:** added an `info` medallion status (accent-tinted) alongside danger/warning/success, so positive/informational confirmations (e.g. the in-app plan-switch confirm) get a first-class medallion too — a small additive enhancement to the EmberFlow-local component; re-extract to the module on the next dialog pass. |
| `components/alert/` | Component | Extracted 2026-07-31 from the billing-legibility milestone. Inline notice (info/success/warning/danger/neutral) with title/icon/action-slot/optional-dismiss; synthesized from Tremor Callout + shadcn Alert + Mantine Alert + HeroUI Alert, rebuilt on Ember's soft/strong semantic tokens. Proven across 5 notice types + the failed-payment post-revoke message + the app-wide dunning banner before extraction. |
| `components/loaders/` | Components | Extracted 2026-07-31 from the premium-experience milestone. `EmberSpinner` (masked-conic warm comet, the default region loader) + `RouteProgress` (navigation top-bar). CSS-only, brand-agnostic; the brand-coupled `BrandLoader`/`ProActivation`/`AppEntrance`/`EmberLogo` from the same pass stayed EmberFlow-local. |
